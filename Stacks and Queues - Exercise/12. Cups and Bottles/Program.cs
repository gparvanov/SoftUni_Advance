        ...
        static void Main(string[] args)
        {
            ...
            while(cups.Any() && bottles.Any())
            {
                ...
                while (currentCup > 0 && bottles.Any())
                {
                    ...
                    if(currentCup < 0) totalWastedWater += Math.Abs(currentCup);                    
                }
            }
            if (cups.Any()) Console.WriteLine($"Cups: {string.Join(" ", cups)}");            
            else Console.WriteLine($"Bottles: {string.Join(" ",bottles)}");
